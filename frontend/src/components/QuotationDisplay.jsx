...
const handleDelete = async (id, onSuccess) => {
  ...
  try {
    const result = await api.deleteMaster('quotations', id);
...
const handlePrint = (row) => {
  ...
  const html = `<html><head><title>Quotation - ${row.s_no || ''}</title><style>body{font-family:Arial;padding:20px}table{border-collapse:collapse;width:100%}td,th{border:1px solid #ccc;padding:8px}</style></head><body><h2>Quotation</h2><table><tr><th>S.No</th><td>${row.s_no||''}</td></tr><tr><th>Date</th><td>${row.date||''}</td></tr><tr><th>Customer</th><td>${row.customer||''}</td></tr></table></body></html>`;
